...
async def process_job_mock(job: dict, stats: BenchmarkStats):
    ...
    num_chunks = 10
    map_latencies = [STAGES["map_chunk"]["mock_latency"] + random.uniform(-0.2, 0.5) for _ in range(num_chunks)]
...
async def run_benchmark(num_jobs: int, num_workers: int):
    ...
    worker_tasks = [asyncio.create_task(worker_loop(i, stats, active_ai_tasks)) for i in range(num_workers)]
