...
class GitHubService:
    ...
    async def post_review(
        ...
        for comment in formatted_comments:
            try:
                await self._send_review_payload(url, headers, commit_id, "", [comment])
            except Exception as e:
                logger.error("Failed to post comment %s: %s", comment, e)

    async def create_check_run(self, repo: str, head_sha: str, token: str) -> int:
    ...
    async def update_check_run(
        self, repo: str, check_run_id: int, token: str, conclusion: str, output: dict[str, Any]
    ) -> None:
