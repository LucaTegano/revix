...
def upgrade() -> None:
    ...
    op.add_column("jobs", sa.Column("fence_token", sa.Integer(), server_default="0", nullable=False))
    ...
    op.create_index("idx_references_repo_symbol", "repo_references", ["repo_full_name", "symbol_name"])
    ...
    op.add_column("jobs", sa.Column("created_date", sa.Date(), server_default=sa.text("CURRENT_DATE"), nullable=False))
