...
def upgrade() -> None:
    ...
    op.create_index("idx_jobs_queue", "jobs", ["scheduled_at", sa.text("priority DESC")], postgresql_where=sa.text("status = 'pending'"))
    op.create_index("idx_jobs_heartbeat", "jobs", ["heartbeat_at"], postgresql_where=sa.text("status = 'processing'"))
