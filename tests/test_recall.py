...
CASES = [
    ...
    (
        ...
        "process",
        None,
        False,  # Runtime, excluded from recall
    ),
    (
        "importlib",
...
def test_static_recall():
    static_cases = [
        (name, src, sym, line) for name, src, sym, line, detectable in CASES if detectable
    ]
    ...
    for name, source, symbol_name, expected_line in static_cases:
        ...
        assert found_ref, (
            f"Missed static call in '{name}': expected {symbol_name} at line {expected_line}"
        )
        found += 1
...
def test_dynamic_dispatch_not_claimed():
    ...
    """
    dynamic_cases = [
        (name, src, sym) for name, src, sym, line, detectable in CASES if not detectable
    ]
